...
def execute_export(rom_path: Path, plan: ExportPlan, dest_dir: Path,
    ...
    if not plan.steps:
        ...
        src_ext = rom_path.suffix.lower()
        tgt_ext = Path(plan.target_name).suffix.lower()
        # Source is an archive but target is a loose file — extract the ROM
        if src_ext in {".zip", ".7z"} and tgt_ext not in {".zip", ".7z"}:
    ...
    for step in plan.steps:
        ...
        elif step.name == "loose_file":
            loose_name = step.params.get("target_name", plan.target_name)
    ...
    # loose_file step: write ROM data as a standalone file (disc systems)
    if loose_name:
        out_path = dest_dir / loose_name
        out_path.write_bytes(rom_data)
        return out_path
            ...
            with py7zr.SevenZipFile(rom_path, "r") as szf:
                ...
                if candidates:
                    ...
                    with tempfile.TemporaryDirectory() as tmp:
                        szf.extract(path=tmp, targets=[best.filename])
                        extracted = Path(tmp) / best.filename
                        if extracted.exists():
